...
export default function RBACSidebar({ user, menuItems }: { user?: SidebarUser | null; menuItems: MenuItem[] }) {
  ...
  return (
    <aside className="flex h-full w-72 flex-col bg-linear-to-b from-[#6F3BB7] to-[#4E2788] text-white shadow-xl shadow-[#4E2788]/25">
      ...
      <div className="border-t border-white/15 px-6 py-7">
        <div className="mb-7">
          ...
          <div className="mt-1 truncate text-sm font-medium text-[#F4E7B0]">{user?.email ?? 'User'}</div>
        </div>

        <button
          type="button"
          onClick={() => signOut({ callbackUrl: "/signin" })}
          className="flex w-full items-center gap-3 rounded-xl px-1 py-2 text-left text-sm font-bold text-[#F4E7B0] transition hover:text-white"
        >
